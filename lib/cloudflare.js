...
export default class CloudflareClient {
  ...
  /**
   * Upload an image to Cloudflare Images from a local file
   * @param {string} filePath - The path to the local image file
   * @returns {Promise<object>} The upload result
   */
  async uploadLocalImage(filePath) {
    // Import fs here to keep the module structure clean
  ...
  getImageUrl(imageId, variant = 'public') {
    ...
    // Check if the deliveryUrl already contains placeholders
    if (baseUrl.includes('<image_id>') || baseUrl.includes('<variant_name>')) {
      // If it contains placeholders, replace them with the actual values
      return baseUrl
        .replace(/<image_id>/g, imageId)
        .replace(/<variant_name>/g, variant);
    } else {
